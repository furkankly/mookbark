...
#[post("/token?<redirect_uri>&<code>&<state>")]
pub async fn token(
    redirect_uri: String,
    code: String,
    state: String,
    ...
    // Set the global "state/pkce_verifier" under /login and compare it to this query param
    println!("state: {state}");
    ...
    let client = BasicClient::new(
        ...
        // A dummy url as we don't generate an auth url in the server
